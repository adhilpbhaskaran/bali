...
export default function PackageCard({ package: pkg }: PackageProps) {
  ...
  // Generate semantic URL based on package ID
  const generatePackageUrl = () => {
    // Always use the ID for routing to ensure consistency
    // The [id] route param in Next.js will handle the actual package fetching
    return `/packages/${pkg.id}`;
  };
  
  return (
    <Link href={generatePackageUrl()} className="block h-full">
      <div className="bento-card overflow-hidden group hover:scale-[1.02] transition-all duration-300 flex flex-col h-full cursor-pointer">
        {/* Package Image */}
        ...
        <div className="p-3 sm:p-4 flex flex-col flex-grow">
          ...
          <div className="mb-2 sm:mb-3 h-8 sm:h-10 overflow-hidden">
            <p className="text-white/70 text-xs sm:text-sm line-clamp-2">{pkg.shortDescription}</p>
          </div>
          ...
          <div className="flex justify-between items-center mt-auto pt-1">
            ...
            <div className="btn-secondary flex-shrink-0 flex items-center text-xs sm:text-sm py-1 px-2 sm:py-1.5 sm:px-3">
